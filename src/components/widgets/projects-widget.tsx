        ...
        <div className="container mx-auto px-4">
          ...
          {layoutConfig.showFilters && categories.length > 0 && (
            <div className="flex flex-wrap gap-2 justify-center mb-8">
              <Button variant="outline" size="sm" className="bg-amber-600 text-white">All</Button>
              {categories.map(cat => (
          ...
          <div className={`grid ${gridCols[layoutConfig.columns as keyof typeof gridCols] || gridCols[3]} gap-6`}>
            {displayProjects.map((project) => (
              <Card key={project.id} className="overflow-hidden group cursor-pointer hover:shadow-lg transition-shadow">
                ...
                <CardContent className="p-4">
                  <span className="text-xs font-medium text-amber-600 uppercase">{project.category}</span>
                  <h3 className="font-semibold mt-1">{project.title}</h3>
                  <p className="text-sm text-stone-500 mt-2 line-clamp-2">{project.description}</p>
                </CardContent>
              </Card>
            ))}
          </div>
                    ...
                    <Card className="overflow-hidden">
                      <div className="relative h-64 md:h-96 bg-stone-200">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                          <span className="text-xs font-medium text-amber-400 uppercase">{project.category}</span>
                          <h3 className="font-semibold text-xl mt-1">{project.title}</h3>
                          <p className="text-sm text-stone-300 mt-2">{project.description}</p>
                        </div>
                      </div>
                    </Card>
                  </div>
    ...
    <section className="py-16 bg-stone-50">
      <div className="container mx-auto px-4">
        ...
        <div className={`${masonryCols[layoutConfig.columns as keyof typeof masonryCols] || masonryCols[3]} gap-6 space-y-6`}>
          {displayProjects.map((project) => (
            <Card key={project.id} className="overflow-hidden group cursor-pointer hover:shadow-lg transition-shadow break-inside-avoid">
              <div className="relative h-48 bg-stone-200">
                {project.image ? (
                  <Image src={project.image} alt={project.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
                ) : (
